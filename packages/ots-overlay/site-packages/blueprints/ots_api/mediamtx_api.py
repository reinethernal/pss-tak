...
def mediamtx_webhook():
    ...
    if event == "init":
            ...
            for path in paths:
                r = requests.post(
                    "{}/v3/config/paths/add/{}".format(
                        app.config.get("OTS_MEDIAMTX_API_ADDRESS"), path.path
                    ),
                ...
                if not video_stream:
                    ...
                    r = requests.get(
                        "{}/v3/config/paths/get/{}".format(
                            app.config.get("OTS_MEDIAMTX_API_ADDRESS"), path["name"]
                        )
                    )
        ...
        if video_stream:
            video_stream.ready = event == "ready"
            try:
                settings = json.loads(video_stream.mediamtx_settings or "{}")
            except json.JSONDecodeError:
                settings = {}
            settings["record"] = True
            video_stream.mediamtx_settings = json.dumps(settings)
            db.session.add(video_stream)
            db.session.commit()
            r = requests.patch(
                "{}/v3/config/paths/patch/{}".format(
                    app.config.get("OTS_MEDIAMTX_API_ADDRESS"), path
                ),
                json=settings,
            )
            ...
            mediamtx_settings.record.data = True
        ...
        if event == "ready":
            os.makedirs(
                os.path.join(
                    app.config.get("OTS_DATA_FOLDER"), "mediamtx", "recordings", video_stream.path
                ),
                exist_ok=True,
            )
            ...
            if setting == "source":
                ...
                # Run ffmpeg and yt-dlp for live YouTube videos
                    ...
                    yt_dlp_path = os.path.join(
                        pathlib.Path.home(), ".opentakserver_venv", "bin", "yt-dlp"
                    )
                    settings["runOnDemand"] = (
                        f"sh -c 'ffmpeg -re -i \"$({yt_dlp_path} -g {source})\" -c:v copy -f rtsp rtsp://127.0.0.1:8554/$RTSP_PATH'"
                    )
                continue
...
def delete_stream():
    try:
        ...
        r = requests.delete(
            "{}/v3/config/paths/delete/{}".format(app.config.get("OTS_MEDIAMTX_API_ADDRESS"), path)
        )
